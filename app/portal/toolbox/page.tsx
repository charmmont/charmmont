...
const TOOL_TYPE_LABELS: Record<string, string> = {
  ...
  self_discovery_questionnaire: 'Self-discovery',
    ...
    <PortalShell role="practitioner" name={profile?.full_name ?? user.email ?? ''}>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[#6B6B65] text-sm mt-1">Your library of reusable coaching tools.</p>
        </div>
        <Link
      ...
      {tools && tools.length > 0 ? (
        ...
        <div className="bg-white rounded-2xl p-16 text-center">
          <p className="text-[#6B6B65] text-sm italic mb-4">No tools created yet.</p>
          <p className="text-[#6B6B65] text-sm mb-8">
            Create your first tool to start assigning exercises to clients.
          </p>
          <Link
            href="/portal/toolbox/new"
